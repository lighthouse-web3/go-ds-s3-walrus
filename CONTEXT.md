...
- Index record: `{ blob_id, size, deletable, end_epoch, expires_at }` per `ds.Key`.
...
- Walrus-first, index-second write ordering (a failure leaks a recoverable blob, never a
  dangling index row).
- **No block packing in v1** (one IPFS block = one Walrus blob), but schema/design leave room
  for it.
- **Epoch renewal via re-upload** (HTTP-only, no Sui key): background worker re-uploads blobs
  near `expires_at`. Enabled when `epochDurationSeconds` + `renewIntervalSeconds` are set.
...
- Built against **kubo v0.30.0**, **go 1.22**.

## Repo layout

```
go-ds-s3-walrus/
  walrus.go      # WalrusDatastore: ds.Datastore + ds.Batching (package walrusds, at root)
  client.go      # context-aware Walrus HTTP client (store/read, retries, failover)
  index.go       # Index interface + postgresIndex (auto-migrate, upsert, get, delete, list, renewal queries)
  renewal.go     # background epoch-renewal worker
  plugin/
...
4. **Per-block cost** — one Sui/publisher operation per IPFS block is expensive at scale;
   block-packing (multiple blocks per Walrus blob, store `offset`/`length`) is the planned
   optimization.
